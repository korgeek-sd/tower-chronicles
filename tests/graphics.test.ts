...
test('그래픽 02: 철맥 일반 15종과 보스 5종을 포함한 현재 그래픽 카탈로그를 조회한다',()=>{assert.equal(graphicFor('ore',start().expedition!.monster)?.id,'muan_miner');assert.equal(MONSTER_GRAPHICS.length,23);for(const name of ['쇄철턱 굴혈수','흑맥갑주 파쇄충','울림포식자','심층 권양감독체','철심 맥동체'])assert.ok(graphicFor('ore',{name}));});
...
test('그래픽 05: HUD는 실제 HP만 사용',()=>{const e=start().expedition!;e.monster.currentHp=21;assert.deepEqual(monsterHud(e.monster),{name:e.monster.name,current:21,max:38,percent:21/38*100});});
...
test('현재 10층 구조는 철맥 배경 5종을 2층 단위로 매핑한다',()=>{for(let floor=1;floor<=10;floor++)assert.equal(backgroundFor('ore',floor),`assets/backgrounds/ore/t${Math.ceil(floor/2)}.png`);for(const t of ['leather','gem','kaleon'] as const)for(let floor=1;floor<=10;floor++)assert.equal(backgroundFor(t,floor),`assets/backgrounds/${t}/t1.png`);});

...
test('그래픽 02: 철맥과 칼레온 일반·보스를 포함한 현재 그래픽 카탈로그를 조회한다',()=>{assert.equal(graphicFor('ore',start().expedition!.monster)?.id,'goblin_miner');assert.equal(MONSTER_GRAPHICS.length,33);for(const name of ['쇄철턱 굴혈수','흑맥갑주 파쇄충','울림포식자','심층 권양감독체','철심 맥동체'])assert.ok(graphicFor('ore',{name}));for(const name of ['성흔을 품은 치유사','대속 십자가 운반자','희생의 집전관','거짓 구원의 사도','칼레온, 녹빛 구원자'])assert.ok(graphicFor('kaleon',{name}));for(const name of ['황야 멧돼지','무리 추적자','의식의 상처입은 자','청소부 하이에나','알파의 아가리'])assert.ok(graphicFor('leather',{name}));for(const name of ['핏갈기 추적자','붉은턱 가죽포식자','송곳니 무리어미','성소 발톱주교','적아의 주인'])assert.ok(graphicFor('leather',{name}));});
...
test('현재 10층 구조는 각 탑에서 단일 배경 tier를 조회한다',()=>{for(const t of ['ore','leather','gem','kaleon'] as const)for(let floor=1;floor<=10;floor++){const expected=t==='kaleon'?'assets/backgrounds/kaleon/t1.webp':`assets/backgrounds/${t}/t1.png`;assert.equal(backgroundFor(t,floor),expected);}});

...
test('그래픽 02: 철맥과 칼레온 일반·보스를 포함한 현재 그래픽 카탈로그를 조회한다',()=>{assert.equal(graphicFor('ore',start().expedition!.monster)?.id,'goblin_miner');assert.equal(MONSTER_GRAPHICS.length,23);for(const name of ['쇄철턱 굴혈수','흑맥갑주 파쇄충','울림포식자','심층 권양감독체','철심 맥동체'])assert.ok(graphicFor('ore',{name}));for(const name of ['성흔을 품은 치유사','대속 십자가 운반자','희생의 집전관','거짓 구원의 사도','칼레온, 녹빛 구원자'])assert.ok(graphicFor('kaleon',{name}));});

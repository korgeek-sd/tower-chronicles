...
import {BESTIARY_ENTRIES} from '../src/game/data/bestiary.ts';
...
test('그래픽 02: 철맥과 천광 확장분을 포함한 현재 그래픽 카탈로그를 조회한다',()=>{assert.equal(graphicFor('ore',start().expedition!.monster)?.id,'goblin_miner');assert.ok(MONSTER_GRAPHICS.length>=BESTIARY_ENTRIES.length);for(const name of ['쇄철턱 굴혈수','흑맥갑주 파쇄충','울림포식자','심층 권양감독체','철심 맥동체'])assert.ok(graphicFor('ore',{name}));});

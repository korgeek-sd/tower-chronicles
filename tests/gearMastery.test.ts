...
test('숙련 04: 현재 착용 가능 티어와 같은 장비만 다음 티어를 진행한다',()=>{const s=fullSet(10);s.gearMastery.dagger.unlockedTier=3;s.items.find(i=>i.id==='dagger')!.tier=1;reward(s,()=>.9);assert.equal(s.gearMastery.dagger.progress,0);});
...
test('v2 저장은 숙련도와 고정 장비를 추가해 현재 v21까지 안전 이전한다',()=>{const {repo,mem}=memory(),s=fullSet(),old:any=withHistoricalTickets(JSON.parse(JSON.stringify(s)));old.gold=old.silver;delete old.silver;old.expedition.loot.gold=old.expedition.loot.silver;delete old.expedition.loot.silver;old.version=2;delete old.gearMastery;delete old.expedition.equipment;delete old.expedition.returnRequested;const raw=JSON.stringify(old);mem.set(SAVE_KEY,raw);const n=repo.load();assert.equal(n.version,21);assert.equal(n.gearMastery.sword.unlockedTier,1);assert.deepEqual(n.expedition!.equipment,n.equipped);assert.equal(mem.get(MASTERY_BACKUP_KEY),raw);});

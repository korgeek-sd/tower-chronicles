...
const v5=(active=true)=>{const s:any=active?bossRun():initialState();s.version=5;if(s.expedition)delete s.expedition.bossTracking;return withHistoricalTickets(s);};
...
test('보스 v24: legacy challenge/decline도 같은 이벤트 결과를 통해 한 번만 처리한다',()=>{const event=kill(bossRun(),()=>0),id=event.expedition!.events.pendingEvent!.instanceId;const challenged=challengeBoss(event);assert.strictEqual(challengeBoss(challenged),challenged);const battle=continueEvent(challenged,id);assert.equal(battle.expedition!.monster.name,'철심 맥동체');assert.equal(battle.expedition!.events.activeBossId,'iron_core_pulsator');const declined=declineBoss(event),normal=continueEvent(declined,id);assert.notEqual(normal.expedition!.monster.name,'철심 맥동체');assert.equal(normal.expedition!.bossTracking.progress,0);});

...
test('SAVE EXPORT 02: 이전 v18 원본 저장도 가져오기 과정에서 현재 형식으로 이전한다',()=>{const legacy:any=structuredClone(initialState());legacy.version=18;const imported=parseSaveImport(JSON.stringify(legacy));assert.equal(imported.version,22);assert.deepEqual(imported.silver,legacy.silver);});

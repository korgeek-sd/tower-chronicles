...
import {EVENT_ASSETS} from '../src/game/events/catalog';
...
const query=new URLSearchParams(location.search),fixture=DEV_EVENTS.find(x=>x.id===query.get('fixture'))??DEV_EVENTS[3];
const d=structuredClone(fixture);if(query.has('long')){d.title='버려진 광부의 가방과 무너진 지지대 아래에서 발견한 오래된 탐사 기록';d.description='희미한 불빛 사이로 남겨진 흔적을 발견했습니다. '.repeat(8);d.choices.push({id:'third',label:'더 살펴본 뒤 기록을 남기고 주변 흔적을 확인한다',description:'선택 조건을 확인하는 세 번째 행동입니다.',effects:[]},{id:'fourth',label:'네 번째 행동',effects:[],conditions:[{kind:'PLAYER_HP_BELOW',ratio:0}]});}if(query.has('noPreview'))delete d.rewardPreview;if(query.has('noArt'))delete d.imageAssetKey;if(query.has('brokenArt')){d.imageAssetKey='qa-broken';EVENT_ASSETS['qa-broken']='./assets/missing-qa.png';}DEV_EVENTS[DEV_EVENTS.findIndex(x=>x.id===d.id)]=d;
function seed(){const s=initialState();s.tickets.ore[9]=1;const n=enter(s,'ore',d.type==='BOSS'?10:1);n.expedition!.events=initialEvents('test');n.expedition!.monster.currentHp=0;openEvent(n,d,()=>.1,d.type==='BOSS'?'mining_ogre':null);return n;}
function Qa(){const [s,set]=useState(seed);const e=s.expedition!;return <div className={'app '+(e.events.pendingEvent?'event-mode':'battle-mode')}><main>{e.events.pendingEvent?<EventScreen key={e.events.pendingEvent.instanceId+e.events.pendingEvent.state} game={s} onHome={()=>{}} onChoice={(id,c)=>set(n=>resolveEvent(n,id,c))} onContinue={id=>set(n=>continueEvent(n,id,()=>.99))}/>:<BattleScreen game={s} onHome={()=>{}} onBasicAttack={()=>set(n=>basicAttack(n,()=>.99))} onSkill={()=>{}} onFlee={()=>{}}/>}<output id="qa-state" style={{display:'none'}}>{JSON.stringify(s)}</output></main><nav>{['거점','가방','거래소','조합','제작','장비'].map(x=><button key={x}>{x}</button>)}</nav></div>}

...
function oldV3(active=true){
 ...
 const value:any=withHistoricalTickets(JSON.parse(JSON.stringify(active?requestReturn(enter(s,'ore',1)):s)));
...
test('v21에 중복 gold 필드가 있으면 세 위치 모두 거부',()=>{for(const location of ['permanent','active','result']){const s:any=location==='result'?leave(enter(initialState(),'ore',1)):enter(initialState(),'ore',1);if(location==='permanent')s.gold=0;else if(location==='active')s.expedition.loot.gold=0;else s.lastExpedition.loot.gold=0;assert.equal(validSave(s),false);}});

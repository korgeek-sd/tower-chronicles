...
import {initialCosmetics,registerAppearance,selectAppearance,selectTitle} from '../src/game/engine/cosmetics.ts';
...
test('외형 15-17: 거점에서 해금 외형을 적용하고 미해금·원정 중 변경은 차단한다',()=>{const s=initialState();s.cosmetics.unlockedAppearanceIds.push('ashen');assert.equal(selectAppearance(s,'ashen',catalog).cosmetics.selectedAppearanceId,'ashen');assert.equal(selectAppearance(initialState(),'ashen',catalog).cosmetics.selectedAppearanceId,'default');const active=enter(s,'ore',1),blocked=selectAppearance(active,'ashen',catalog);assert.equal(blocked.cosmetics.selectedAppearanceId,'default');assert.equal(blocked.notice,'원정 중에는 외형을 변경할 수 없습니다.');});
test('외형 18-20: 외형 등록·선택은 장비와 능력치를 바꾸지 않고 사망 뒤에도 유지된다',()=>{let s=initialState();s.cosmetics.appearanceItems.ashen=1;const before=stats(s);s=selectAppearance(registerAppearance(s,'ashen',catalog),'ashen',catalog);assert.deepEqual(stats(s),before);assert.deepEqual(s.equipped,initialState().equipped);assert.equal(leave(enter(s,'ore',1),true).cosmetics.selectedAppearanceId,'ashen');});
test('외형 21-23: 칭호는 보유해야 선택 가능하고 해제할 수 있으며 능력치에 영향이 없다',()=>{const s=initialState(),before=stats(s);assert.equal(selectTitle(s,'test_title',catalog).cosmetics.selectedTitleId,null);s.cosmetics.unlockedTitleIds.push('test_title');const selected=selectTitle(s,'test_title',catalog);assert.equal(selected.cosmetics.selectedTitleId,'test_title');assert.deepEqual(stats(selected),before);assert.equal(selectTitle(selected,null,catalog).cosmetics.selectedTitleId,null);});
test('외형 24-26: 외형·칭호 선택은 저장 후 복구되고 원정 중 칭호 변경은 막힌다',()=>{const {repo}=memory();let s=initialState();s.cosmetics.unlockedAppearanceIds.push('ashen');s.cosmetics.unlockedTitleIds.push('test_title');s=selectTitle(selectAppearance(s,'ashen',catalog),'test_title',catalog);repo.save(s);assert.deepEqual(repo.load().cosmetics,s.cosmetics);const blocked=selectTitle(enter(s,'ore',1),null,catalog);assert.equal(blocked.cosmetics.selectedTitleId,'test_title');assert.equal(blocked.notice,'원정 중에는 칭호를 변경할 수 없습니다.');});

...
export interface MarketStorageEntry {storageId:string;tradeId:string;side:MarketSide;itemId:string;quantity:number;silver:number;createdAt:number;gear?:Item}
export interface MarketState {traderCertified:boolean;ownerId:string;gold:number;orders:MarketOrder[];trades:MarketTrade[];storage?:MarketStorageEntry[];nextOrderId:number;nextTradeId:number;nextSequence:number;nextStorageId?:number}
...
export interface BestiaryProgress {encounters:number;defeats:number}
export interface BestiaryState {entries:Record<string,BestiaryProgress>}

...
import {createStrongholdRuntime} from './resourceStronghold';
let defaultMode:EventMode='production';
export function configureEventMode(mode:EventMode){defaultMode=mode;}
export function initialEvents(mode:EventMode=defaultMode):ExpeditionEvents{return {phase:'BATTLE',mode,pendingEvent:null,sequence:0,recentEventIds:[],bossKillCountThisExpedition:0,activeBossId:null,stronghold:null};}
...
export function openEvent(s:GameState,definition:ExpeditionEventDefinition,rng:Rng=random,bossId:string|null=null,now=Date.now()){const e=s.expedition;if(!e||e.events.pendingEvent||!eligible(s,definition))return;const ev=e.events;if(definition.type==='BOSS'){if(!bossId||bossId!==bossIdFor(e.tower,e.floor))return;e.bossTracking.progress=0;e.bossTracking.pendingBossId=null;e.bossTracking.encounterReason=null;}
 ev.phase='EVENT';e.phase='BATTLE_END';e.monsterRuntime=null;clearReactivePrepared(e);ev.pendingEvent={instanceId:'event-'+(ev.sequence=++s.nextId),eventId:definition.id,bossId,state:'CHOICE',choiceId:null,outcomeId:null,resultText:'',resultLines:[],next:'NORMAL',randomValue:unit(rng),expiresAt:definition.decisionMs?now+definition.decisionMs:null};ev.recentEventIds=[definition.id,...ev.recentEventIds].slice(0,EVENT_BALANCE.repeatWindow);log(s,definition.title+' · 선택 대기');}

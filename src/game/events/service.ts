...
import {tierOf,getMaterialGrade,TOWERS,POTIONS} from '../data/config';
...
function apply(s:GameState,effect:EventEffect,lines:string[]){const e=s.expedition!;switch(effect.kind){
 ...
 case 'ADD_TEMP_LOOT':{const loot=effect.loot;if(loot.kind==='MATERIAL'){const tower=loot.tower==='CURRENT'?e.tower:loot.tower,grade=loot.tier==='CURRENT'?getMaterialGrade(e.floor):loot.tier;e.loot.materials[tower][grade-1]+=loot.amount;lines.push(grade+'등급 '+TOWERS[tower].material+' +'+loot.amount+' · 원정 임시 보관');}else{e.loot.items[loot.itemId]=(e.loot.items[loot.itemId]??0)+loot.amount;lines.push('발견물 +'+loot.amount+' · 원정 임시 보관');}break;}

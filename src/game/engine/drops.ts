...
import {LEATHER_T1_MONSTER_BY_ID,leatherFloorContent} from '../data/leatherSpire';
export function monsterFor(tower:Tower,floor:number,rng:()=>number=()=>0):Monster {const baseHp=COMBAT.monsterHp+(floor-1)*COMBAT.hpPerFloor,baseAttack=COMBAT.monsterAttack+(floor-1)*COMBAT.attackPerFloor,baseSpeed=COMBAT.monsterSpeed+floor*COMBAT.speedPerFloor,floorContent=tower==='ore'?ironFloorContent(floor):tower==='kaleon'?kaleonFloorContent(floor):tower==='leather'?leatherFloorContent(floor):undefined,pool=floorContent?.normalPool,byId=tower==='ore'?IRON_T1_MONSTER_BY_ID:tower==='kaleon'?KALEON_T1_MONSTER_BY_ID:tower==='leather'?LEATHER_T1_MONSTER_BY_ID:undefined,content=pool?.length&&byId?byId[pool[Math.min(pool.length-1,Math.floor(rng()*pool.length))]]:undefined,hp=Math.round(baseHp*(content?.hpMultiplier??1));return {definitionId:content?.id??tower+':default',name:content?.displayName??TOWERS[tower].monster,hp,currentHp:hp,attack:baseAttack*(content?.attackMultiplier??1),defense:COMBAT.monsterDefense+(floor-1)*COMBAT.defensePerFloor,speed:baseSpeed*(content?.speedMultiplier??1),skillPower:1};}

...
export function monsterFor(tower:Tower,floor:number,rng:()=>number=()=>0):Monster {const baseHp=COMBAT.monsterHp+(floor-1)*COMBAT.hpPerFloor,baseAttack=COMBAT.monsterAttack+(floor-1)*COMBAT.attackPerFloor,baseSpeed=COMBAT.monsterSpeed+floor*COMBAT.speedPerFloor,pool=tower==='ore'&&floor<=10?ironFloorContent(floor)?.normalPool:tower==='leather'&&floor<=10?redFloorContent(floor)?.normalPool:null,content=pool?.length?(IRON_T1_MONSTER_BY_ID[pool[Math.min(pool.length-1,Math.floor(rng()*pool.length))]]??RED_T1_MONSTER_BY_ID[pool[Math.min(pool.length-1,Math.floor(rng()*pool.length))]]):undefined,hp=Math.round(baseHp*(content?.hpMultiplier??1));return {definitionId:content?.id??tower+':default',name:content?.displayName??TOWERS[tower].monster,hp,currentHp:hp,attack:baseAttack*(content?.attackMultiplier??1),defense:COMBAT.monsterDefense+(floor-1)*COMBAT.defensePerFloor,speed:baseSpeed*(content?.speedMultiplier??1),skillPower:1};}

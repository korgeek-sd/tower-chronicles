...
import {IRON_T1_MONSTER_BY_ID,ironFloorContent,IRON_BOSS_REWARDS} from '../data/ironSpire';
...
export function reward(s:GameState,rng:()=>number=Math.random){
  ...
  if(!e)return;
  const tier=tierOf(e.floor),baseSilver=COMBAT.silverBase+e.floor*COMBAT.silverPerFloor,bossReward=e.events.activeBossId?IRON_BOSS_REWARDS[e.events.activeBossId]:undefined,silver=baseSilver+(bossReward?.silverBonus??0),materialAmount=COMBAT.materialAmount+(bossReward?.materialBonus??0);
  e.kills++;
  awardEquippedMastery(s);
  e.loot.silver+=silver;
  e.loot.materials[e.tower][tier-1]+=materialAmount;
  log(s,e.monster.name+' 처치! '+tier+'T '+TOWERS[e.tower].material+' ×'+materialAmount+' · Silver +'+silver+(bossReward?' · 보스 전리품':'')+' (원정 임시 보관)');

...
import {leatherFloorContent} from '../data/leatherSpire';
...
export const contentReady=(tower:Tower,floor:number)=>tower==='ore'?!!ironFloorContent(floor)?.normalPool.length:tower==='kaleon'?!!kaleonFloorContent(floor)?.normalPool.length:tower==='leather'?!!leatherFloorContent(floor)?.normalPool.length:false;

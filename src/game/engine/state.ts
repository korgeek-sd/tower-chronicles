...
import {CONFIG,STARTER,TOWERS,WEAPONS,EQUIPMENT,PASSIVES,towerIds,GEAR_MASTERY_KEYS,GEAR_MASTERY_NAMES,SLOT_EQUIPMENT_BONUS} from '../data/config';
...
export function stats(s:GameState,equipment=s.equipped):Stats {const w=equippedItem(s,'weapon',equipment);const d=WEAPONS[weaponOf(s,equipment)];const scale=w?(w.id==='starter'?STARTER.scale:1)*w.tier*(1+w.enhancement*.1):.4;const armor=equippedItem(s,'armor',equipment),boots=equippedItem(s,'boots',equipment);const weaponBonus=SLOT_EQUIPMENT_BONUS.weapon,armorBonus=SLOT_EQUIPMENT_BONUS.armor,bootsBonus=SLOT_EQUIPMENT_BONUS.boots,accessoryBonus=SLOT_EQUIPMENT_BONUS.accessory;const weaponTier=w?.tier||0,armorTier=armor?.tier||0,bootsTier=boots?.tier||0,accessoryTier=equippedItem(s,'accessory',equipment)?.tier||0;return {hp:CONFIG.baseHp+(armorTier||0)*EQUIPMENT.armor.hp*(armorBonus.hp??1)+(bootsTier||0)*EQUIPMENT.boots.hp*(bootsBonus.hp??1),attack:CONFIG.baseAttack+d.attack*scale*(weaponBonus.attack??1),defense:CONFIG.baseDefense+d.defense*scale+(armorTier||0)*EQUIPMENT.armor.defense*(armorBonus.defense??1),speed:d.speed+(bootsTier||0)*EQUIPMENT.boots.speed*(bootsBonus.speed??1),skillPower:d.skillPower*(accessoryBonus.skillPower??1)}};

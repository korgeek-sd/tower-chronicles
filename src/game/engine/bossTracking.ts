...
export const bossIdFor=(tower:Tower,floor:number):string|null=>tower==='ore'?ironFloorContent(floor)?.bossId??null:tower==='kaleon'?kaleonFloorContent(floor)?.bossId??null:null;
...
export const bossById=(id:string)=>IRON_T1_MONSTER_BY_ID[id]??KALEON_T1_MONSTER_BY_ID[id]??slotBoss(id);
export const bossIdByName=(name:string)=>IRON_T1_MONSTERS.find(m=>m.boss&&m.displayName===name)?.id??KALEON_T1_MONSTERS.find(m=>m.boss&&m.displayName===name)?.id??Object.values(IRON_BOSS_SLOTS).find(slot=>slot.name===name)?.bossId??Object.values(KALEON_BOSS_SLOTS).find(slot=>slot.name===name)?.bossId??null;

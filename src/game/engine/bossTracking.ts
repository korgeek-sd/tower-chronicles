...
export const bossIdFor=(tower:Tower,floor:number):string|null=>tower==='ore'?ironFloorContent(floor)?.bossId??null:tower==='leather'?redFloorContent(floor)?.bossId??null:null;
export const bossById=(id:string)=>IRON_T1_MONSTER_BY_ID[id]??RED_T1_MONSTER_BY_ID[id]??(Object.values(IRON_BOSS_SLOTS).find(slot=>slot.bossId===id)?{id,displayName:Object.values(IRON_BOSS_SLOTS).find(slot=>slot.bossId===id)!.name,boss:true,...IRON_BOSS_STATS[id]}:undefined)??(Object.values(RED_BOSS_SLOTS).find(slot=>slot.bossId===id)?{id,displayName:Object.values(RED_BOSS_SLOTS).find(slot=>slot.bossId===id)!.name,boss:true,...RED_BOSS_STATS[id]}:undefined);

  ...
  return <Screen eyebrow="SILVER SCALE / ORDER DESK" title={item.name} meta={<button className="tc-action secondary slim" onClick={()=>{setSelected(null);setConfirm(false);setFeedback('');}}>목록</button>}>
   <div className="tc-market-item">
    ...
    <section className="tc-market-ticket">
     <Segments items={[['BUY','매수 요청'],['SELL','매도 요청']] as const} value={side} onChange={v=>{setSide(v);setPrice(String((v==='BUY'?ask:bid)??displayLast??''));}} label="주문 방향"/>
    ...
    <div className="tc-market-tape">{displayRecent.length?displayRecent.map(t=><span key={t.tradeId}>{time(t.executedAt)} {money(t.price)}×{t.quantity}</span>):<span>최근 체결 기록 없음</span>}</div>
   </div>

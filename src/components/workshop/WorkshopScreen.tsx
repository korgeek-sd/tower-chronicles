...
export function WorkshopScreen({game,setGame,now,onEnhancement,onMastery}:{game:GameState;setGame:React.Dispatch<React.SetStateAction<GameState>>;now:number;onEnhancement:()=>void;onMastery:()=>void}){
 const [field,setField]=useState<Field>('weapon'),[tier,setTier]=useState(1),[page,setPage]=useState(0),[cancelTarget,setCancelTarget]=useState<CraftJob|null>(null);
 ...
 return <Screen eyebrow="ASSOCIATION WORKSHOP" title="공방" meta={<><button className="tc-action secondary slim" onClick={onEnhancement}>강화</button><button className="tc-action secondary slim" onClick={onMastery}>숙련</button></>}>
  <div className="tc-workshop">
   <div className="tc-work-status">{completed.length?<><div><strong>완료품 {completed.length}개</strong><small>{completed.slice(0,2).map(label).join(' · ')}</small></div><button className="tc-action slim" onClick={()=>setGame(s=>claimAllCraft(s,Date.now()))}>모두 수령</button></>:active?<><div><strong>{label(active)}</strong><small>{FIELDS[active.field]} · {materialName} {active.consumedMaterials}개</small><Meter value={progress} max={1}/></div><b>{left((active.completesAt??now)-now)}</b></>:<><div><strong>작업대 대기</strong><small>제작할 품목을 선택하세요.</small></div><Glyph name="craft"/></>}</div>
   <div className="tc-work-ready">{completed.length>0&&<span>개별 수령: {completed.slice(0,2).map(job=><button key={job.jobId} onClick={()=>setGame(s=>claimCraft(s,job.jobId,Date.now()))}>{label(job)}</button>)}</span>}{active&&<button className="tc-action secondary slim" onClick={()=>setCancelTarget(active)}>현재 제작 취소</button>}</div>

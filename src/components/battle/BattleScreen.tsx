...
import {CombatEffectStrip} from './CombatEffectStrip';
import {MonsterSkillPanel} from './MonsterSkillPanel';
import {logKind} from './report';
...
export function BattleScreen({game,onBasicAttack,onSkill,onPotion,onFlee,onHome,onRevival}:Props){
 const e=game.expedition!,st=stats(game,e.equipment),weapon=weaponOf(game,e.equipment),[menu,setMenu]=useState(false),[itemsOpen,setItemsOpen]=useState(false),[enemyInfoOpen,setEnemyInfoOpen]=useState(false),playerReactive=reactivePreparedSkill(e,'player'),playerShield=activeShield(e,'player'),intel=monsterCombatIntel(e);
 ...
 useEffect(()=>{if(prefs.autoscroll)logBoxRef.current?.scrollTo(0,logBoxRef.current.scrollHeight);},[game.logs.length,prefs.autoscroll]);
 ...
 return <div className="battle-screen immersive-battle">
  <BattleScene expedition={e} combatEvents={game.combatEvents??[]} playerMaxHp={st.hp} appearanceId={game.cosmetics.selectedAppearanceId} titleName={titleById(game.cosmetics.selectedTitleId||'')?.name} speed={prefs.speed} showDamage={prefs.damageNumbers}/>
  ...
  <aside className="enemy-facts"><small>적 능력</small><span>⚔ {Math.round(e.monster.attack)} <em>공격</em></span><span>⬡ {Math.round(e.monster.defense)} <em>방어</em></span><button type="button" className="enemy-info-toggle" onClick={()=>setEnemyInfoOpen(true)}>적 정보</button></aside>
  ...
  {(playerShield||intel.shield)&&<div className="shield-status" aria-label="보호막 상태">{playerShield&&<span className="player-shield"><b>⬢ 내 보호막</b>{effectText(playerShield)}</span>}{intel.shield&&<span className="monster-shield"><b>⬢ 적 보호막</b>{intel.shield.name} · {intel.shield.shieldCurrent??intel.shield.shieldHits}{intel.shield.shieldMax!==undefined?` / ${intel.shield.shieldMax}`:''} · {intel.shield.turns}턴</span>}</div>}
  <CombatEffectStrip effects={intel.effects}/>
  <div className="player-effects" aria-label="플레이어 효과">{buffs.length?buffs.map(effect=><span key={effect.instanceId} title={EFFECTS[effect.effectId]?.description}>{effectText(effect)}</span>):<small>{WEAPONS[weapon].name} 장착 · 효과 없음</small>}</div>
  <MonsterIntentBanner intent={intel.intent}/>
  ...
  {enemyInfoOpen&&<MonsterSkillPanel intel={intel} onClose={()=>setEnemyInfoOpen(false)}/>}
  ...
  {menu&&<section className="battle-drawer" aria-label="전투 메뉴"><div className="drawer-heading"><h2>원정 기록</h2><button onClick={()=>setMenu(false)} aria-label="전투 메뉴 닫기">×</button></div><button onClick={onHome}>거점 / 장비 / 스킬 관리 →</button><ExpeditionLootPanel expedition={e}/><h3>전투 기록</h3><h3>전투 기록</h3>
<div ref={logBoxRef} className="logs" role="log">{game.logs.slice(-8).map((line,i)=><div key={'r'+i} className={logKind(line)?'log-'+(logKind(line) as string):undefined}>{line}</div>)}</div>
<details open={logsOpen} onToggle={e=>setLogsOpen((e.target as HTMLDetailsElement).open)}><summary>{logsOpen?'전체 기록 닫기':'전체 기록 보기 ('+game.logs.length+')'}</summary><div className="logs full" role="log">{game.logs.map((line,i)=><div key={i} className={logKind(line)?'log-'+(logKind(line) as string):undefined}>{line}</div>)}</div></details>
<h3>전투 표시 설정</h3>
<div className="battle-prefs" aria-label="전투 표시 설정"><span>속도</span>{SPEEDS.map(v=><button key={v} type="button" disabled={prefs.speed===v} onClick={()=>updatePrefs({...prefs,speed:v})}>{v}배</button>)}<button type="button" onClick={()=>updatePrefs({...prefs,damageNumbers:!prefs.damageNumbers})}>피해 숫자 {prefs.damageNumbers?'켜짐':'꺼짐'}</button><button type="button" onClick={()=>updatePrefs({...prefs,autoscroll:!prefs.autoscroll})}>자동 스크롤 {prefs.autoscroll?'켜짐':'꺼짐'}</button></div><h3>원정 포션</h3>{potionIds.map(p=><p key={p}>{POTIONS[p].name} · {e.bag[p]}개</p>)}<details><summary>장비 숙련도</summary>{Object.entries(game.gearMastery).map(([id,m])=><p key={id}>{GEAR_MASTERY_NAMES[id as keyof typeof GEAR_MASTERY_NAMES]} · T{m.unlockedTier} · {m.progress}</p>)}</details><p>도망가면 적이 한 번 공격합니다. 생존 시 획득물을 정산하고 귀환합니다.</p><button disabled={!playerTurn} onClick={onFlee}>도망가기</button></section>}

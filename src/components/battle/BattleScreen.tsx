...
export function BattleScreen({game,onBasicAttack,onSkill,onPotion,onFlee,onHome}:Props){
 ...
 return <div className="battle-screen immersive-battle">
  ...
  <div className="battle-location"><img className="navigation-art" src="./assets/ui/navigation/towers.png" alt=""/><h1>{TOWERS[e.tower].name}</h1><b>{e.floor}F <small>/ 50F · T{tierOf(e.floor)}{isBossFloor(e.floor)?' · 보스':null}</small></b></div>
  <div className="battle-route" aria-label={`현재 ${e.floor}층, T${tierOf(e.floor)}`}><div>{Array.from({length:10},(_,i)=><i key={i} className={i===(e.floor-1)%10?'current':i<(e.floor-1)%10?'passed':''}>{i===9?'♜':'◆'}</i>)}</div><p>T{tierOf(e.floor)} · {(tierOf(e.floor)-1)*10+1}~{tierOf(e.floor)*10}층 · 보스 {tierOf(e.floor)*10}층</p></div>

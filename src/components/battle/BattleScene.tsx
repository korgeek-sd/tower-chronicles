...
function Encounter({expedition,combatEvents,playerMaxHp,appearanceId,titleName,speed=1,showDamage=true}:{expedition:Expedition;combatEvents:CombatEvent[];playerMaxHp:number;appearanceId:string;titleName?:string;speed?:number;showDamage?:boolean}){
  ...
  useEffect(()=>{const before=previous.current,monsterDamage=damageBetween(before,expedition),playerEvent=playerVitalBetween(before,expedition),direct=combatEvents.filter(event=>event.id>previousEventId.current);previous.current=expedition;previousEventId.current=combatEvents.at(-1)?.id??previousEventId.current;
    ...
    if(monsterDamage||direct.some(event=>event.target==='monster'))setMonsterHit(true);if(playerEvent?.kind==='damage'||direct.some(event=>event.target==='player'))setPlayerHit(true);const rate=speed>0?speed:1,hitMs=Math.round(SCENE_CONFIG.hitDurationMs/rate),dmgMs=Math.round(SCENE_CONFIG.damageDurationMs/rate);setEvents(x=>showDamage?[...x,...additions].slice(-SCENE_CONFIG.maxDamageLabels):[]);const pulse=seq.current;
    ...
    schedule(()=>{if(seq.current===pulse){setMonsterHit(false);setPlayerHit(false);}},hitMs);
...
export function BattleScene({expedition,combatEvents,playerMaxHp,appearanceId,titleName,speed=1,showDamage=true}:{expedition:Expedition;combatEvents:CombatEvent[];playerMaxHp:number;appearanceId:string;titleName?:string;speed?:number;showDamage?:boolean}){
  return <section className="battle-scene" aria-label="전투 그래픽" style={{'--hit-duration':Math.round(SCENE_CONFIG.hitDurationMs/(speed>0?speed:1))+'ms','--damage-duration':Math.round(SCENE_CONFIG.damageDurationMs/(speed>0?speed:1))+'ms'} as React.CSSProperties}><BackgroundLayer path={backgroundFor(expedition.tower,expedition.floor)}/><div className="scene-status">{expedition.phase==='PLAYER_TURN'?'내 턴 · 행동을 선택하세요':expedition.phase==='MONSTER_TURN'?'적이 행동합니다.':'전투 결과를 처리합니다.'}</div><Encounter key={encounterKey(expedition)} expedition={expedition} combatEvents={combatEvents} playerMaxHp={playerMaxHp} appearanceId={appearanceId} titleName={titleName} speed={speed} showDamage={showDamage}/></section>;
}

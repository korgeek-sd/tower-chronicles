 ...
 return <Screen eyebrow="SAVE / CLOUD" title="저장 관리" meta={<span>v{APP_VERSION}</span>}>
  <div style={{height:'100%',display:'grid',gridTemplateRows:'1.25fr 1fr auto',gap:'6px'}}>
   <section className="tc-panel strong" style={{display:'grid',alignContent:'center',gap:'7px'}}>
    ...
    {session?<><div className="tc-floor-risk" role="status"><b>{stateLabel}</b> · {syncMessage}</div><p style={{margin:0,fontSize:'8px',color:'var(--muted)'}}>플레이 변경은 자동 저장되고, 같은 Google 계정의 다른 기기에서 변경되면 실시간으로 최신 revision을 반영합니다.</p><button className="tc-action secondary" disabled={busy} onClick={()=>void logout()}>로그아웃</button></>:<button className="tc-action" disabled={!onlineConfigured||busy} onClick={()=>signInWithGoogle()}>Google로 계속하기</button>}
   </section>

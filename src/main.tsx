...
function App(){
 ...
 const serverEconomyBusy=useRef(false),settledReceiptKey=useRef(''),confirmedKillCount=useRef(0),onlineRunVersion=useRef(0),recordingAction=useRef(false),onlineCombatNonce=useRef(0);
 ...
 async function restoreServerRun(lease:GameplayLease){
  try{const restored=await restoreOnlineExpedition(lease);if(!restored.active||!restored.run)return;
   ...
   if(combat&&typeof combat.playerHp==='number'&&typeof combat.monsterHp==='number'&&typeof combat.monsterId==='string'&&typeof combat.phase==='string'){
    ...
    setGame(current=>{const next=reconcileOnlineCombatState(current,snapshot),e=next.expedition;if(e){const p=restored.run!.potions;e.bag.healing_lesser=p.lesser??e.bag.healing_lesser;e.bag.healing_standard=p.standard??e.bag.healing_standard;e.bag.healing_greater=p.greater??e.bag.healing_greater;e.bag.healing_supreme=p.supreme??e.bag.healing_supreme;e.bag.revival=p.revival??e.bag.revival;e.kills=restored.run!.confirmedKills;e.bossTracking.progress=restored.run!.bossProgress;e.bossTracking.bossDefeated=restored.run!.bossDefeated;const sh=restored.run!.stronghold;if(sh){e.events.stronghold={instanceId:sh.instanceId,status:sh.status as any,ownerUserId:next.market.ownerId,tower:sh.tower,floor:sh.floor,version:sh.version,captureStartedAt:new Date(sh.captureStartedAt).getTime(),captureEndsAt:new Date(sh.captureEndsAt).getTime(),reward:sh.reward,contestedByUserId:null,contestRemainingMs:null,completedAt:null,abandonedAt:null,deletedAt:sh.status==='DELETED'?Date.now():null};}else e.events.stronghold=null;}return next;});setPage('battle');
   }

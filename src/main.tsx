...
import {CONFIG,EQUIPMENT,WEAPONS,TOWERS,FIELDS,SLOTS,PASSIVES,POTIONS,SKILLS,potionIds,generalPotionIds,towerIds,tierOf,GEAR_MASTERY_KEYS,GEAR_MASTERY_NAMES} from './game/data/config';
...
import './components/battle/expedition.css';
import './components/events/events.css';
import './components/character/journal.css';
type Page='home'|'towers'|'floor'|'battle'|'inventory'|'equipment'|'craft'|'mastery'|'skills'|'jobs'|'cosmetics'|'premium'|'market'|'association'|'settings';
...
const nav:[Page,string,string][]=[['home','⌂','거점'],['inventory','▣','가방'],['market','⚖','거래소'],['association','♜','조합'],['craft','⚒','제작'],['equipment','⚔','캐릭터']];
...
const [inventorySlot,setInventorySlot]=useState<Slot|null>(null);
...
function openInventory(slot?:Slot){setInventorySlot(slot??null);setPage('inventory');}
function move(p:Page){if(exp?.pendingRevival)return;if(p==='inventory')setInventorySlot(null);setPage(p==='towers'&&exp?'battle':p);}
...
return <div className={page==='battle'&&exp?(eventOpen?'app event-mode':'app battle-mode'):['inventory','equipment','jobs','skills','cosmetics'].includes(page)?'app journal-mode':'app'}>
...
</main><nav>{nav.map(([p,_icon,label])=><button key={p} className={(page===p||(p==='towers'&&['floor','battle'].includes(page))||(p==='craft'&&page==='mastery')||(p==='equipment'&&['skills','jobs','cosmetics'].includes(page)))?'active':''} onClick={()=>move(p)}><span><img className="navigation-art" src={`./assets/ui/navigation/${p}.png`} alt=""/></span>{label}</button>)}</nav></div>;
}

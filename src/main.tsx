...
import {CampBagShell} from './components/camp-bag/CampBagShell';
...
import {CONFIG,EQUIPMENT,WEAPONS,TOWERS,FIELDS,SLOTS,PASSIVES,POTIONS,SKILLS,potionIds,generalPotionIds,towerIds,tierOf,GEAR_MASTERY_KEYS,GEAR_MASTERY_NAMES,ENTRY_PERMITS} from './game/data/config';
...
import {MobileScreenPager} from './components/MobileScreenPager';
...
if(page==='home'||page==='inventory')return <CampBagShell game={game} currentPage={page} onNavigate={move} saved={saved} storageError={storageError}>{page==='home'?<HomeScreen game={game} onNavigate={move}/>:<InventoryScreen game={game} setGame={setGame}/>}</CampBagShell>;
...
<main>
<MobileScreenPager pageKey={page} disabled={page==='battle'&&!!exp&&!eventOpen}>
...
</main><nav>{nav.map(([p,_icon,label])=>{const isNew=newNavigationIcons.has(p);return <button key={p} className={(page===p||(p==='towers'&&['floor','battle'].includes(page))||(p==='craft'&&page==='mastery')||(p==='equipment'&&page==='skills'))?'active':''} onClick={()=>move(p)}><span><img className={'navigation-art'+(isNew?' navigation-new-art':'')} src={isNew?`./assets/ui/navigation-new/${p}.png`:`./assets/ui/navigation-relic/${p}.svg`} alt=""/></span>{label}</button>;})}</nav></div>;
}

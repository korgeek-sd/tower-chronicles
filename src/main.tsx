...
import {CONFIG,EQUIPMENT,WEAPONS,TOWERS,FIELDS,SLOTS,PASSIVES,POTIONS,SKILLS,potionIds,towerIds,tierOf,GEAR_MASTERY_KEYS,GEAR_MASTERY_NAMES} from './game/data/config';
...
import {basicAttack,useBattleSkill,useBattlePotion,flee,resolveMonsterTurn} from './game/engine/combat';
...
import {createRepository,SAVE_KEY} from './storage/repository';
...
type Page='home'|'towers'|'floor'|'battle'|'inventory'|'equipment'|'craft'|'mastery'|'skills'|'jobs'|'cosmetics'|'premium'|'market'|'association';
...
useEffect(()=>{if(game.expedition?.phase!=='MONSTER_TURN')return;const id=window.setTimeout(()=>setGame(resolveMonsterTurn),1000);return()=>window.clearTimeout(id);},[game.expedition?.phase]);
...
function move(p:Page){setPage(p==='towers'&&exp?'battle':p);}
...
const recipes=field==='weapon'?Object.keys(WEAPONS):field==='armor'?['armor','boots']:field==='accessory'?Object.keys(PASSIVES):potionIds;
...
<div className="statusline"><span><i className={exp?'live':''}/> {exp?'원정 진행 중':'모험가의 거점'}</span><span>v0.1.27 · {saved}</span></div>
<main>
...
{page==='craft'&&<><div className="section-label">WORKSHOP / 05</div><div className="section-heading"><h1>모험가의 공방</h1><button onClick={()=>setPage('mastery')}>숙련도 →</button></div><div className="tabs">{(Object.keys(FIELDS) as Field[]).map(f=><button key={f} className={field===f?'selected':''} onClick={()=>{setField(f);setTier(1);}}>{FIELDS[f]}</button>)}</div><div className="panel"><label className="setting">제작 티어<select aria-label="제작 티어" value={tier} onChange={e=>setTier(+e.target.value)}>{[1,2,3,4,5].map(t=><option key={t} value={t}>{t}티어 {t>game.mastery[field].unlocked?'· 잠김':''}</option>)}</select></label><p className="muted">{tier}T {TOWERS[materialFor(field)].material} <b>{game.materials[materialFor(field)][tier-1]}개</b> 보유 · 재료 절감 {Math.round((discount(game.mastery[field].crafts)+golden.craftingMaterialReductionBonus)*100)}%</p></div>{recipes.map(kind=>{const title=field==='alchemy'?POTIONS[kind as Potion].name+' 포션 ×10':itemName({id:'preview',kind,tier,enhancement:0});const amount=cost(game,field,tier,now);return <section className="panel recipe" key={kind}><div><h2>{title}</h2><p>{kind in WEAPONS?WEAPONS[kind as keyof typeof WEAPONS].description:kind in PASSIVES?PASSIVES[kind as keyof typeof PASSIVES].description:field==='alchemy'?POTIONS[kind as Potion].description:kind==='armor'?'최대 HP +'+EQUIPMENT.armor.hp*tier+' · 방어 +'+EQUIPMENT.armor.defense*tier:'최대 HP +'+EQUIPMENT.boots.hp*tier+' · 공격속도 +'+(EQUIPMENT.boots.speed*tier).toFixed(1)}</p><small>{tier}T {TOWERS[materialFor(field)].material} {amount}개 필요</small></div><button disabled={!!exp||tier>game.mastery[field].unlocked||game.materials[materialFor(field)][tier-1]<amount} onClick={()=>setGame(s=>startCraft(s,kind,tier,field==='alchemy'?10:1,Date.now()))}>{field==='alchemy'?'10개 제작':game.crafting.jobs.some(j=>j.status==='CRAFTING')&&goldenActive?'대기열에 추가':'제작 시작'}</button></section>;})}<div className="note">장비 성능은 항상 동일합니다. 제작 후 장비 화면에서 직접 장착하세요.</div></>}
{page==='mastery'&&<><div className="section-label">CRAFT MASTERY</div><h1>제작 숙련도</h1><p className="muted">현재 최고 제작 티어를 제작해야 다음 티어 자격이 오릅니다.</p>{(Object.keys(FIELDS) as Field[]).map(f=>{const m=game.mastery[f];return <section className="panel" key={f}><div className="section-heading"><h2>{FIELDS[f]}</h2><span className="badge">{m.unlocked}T 해금</span></div><p>{m.unlocked<5?m.unlocked+'T 제작 → '+(m.unlocked+1)+'T 자격':'모든 제작 티어 해금'}</p><div className="hp mastery"><div style={{width:(m.unlocked===5?100:m.progress/CONFIG.masteryRequired*100)+'%'}}/></div><div className="mini-stats"><span>{m.unlocked<5?m.progress+' / '+CONFIG.masteryRequired:'완료'}</span><span>총 제작 {m.crafts}회</span><span>재료 절감 {Math.round(discount(m.crafts)*100)}%</span></div></section>;})}<div className="note">예: 4T까지 해금했다면 4T 제작만 5T 자격을 올립니다. 낮은 티어 제작은 전체 제작 횟수에 따른 재료 절감에만 반영됩니다.</div></>}
<div className="notice" role="status">{game.notice}</div>
</main><nav>{nav.map(([p,_icon,label])=><button key={p} className={(page===p||(p==='towers'&&['floor','battle'].includes(page))||(p==='craft'&&page==='mastery')||(p==='equipment'&&page==='skills'))?'active':''} onClick={()=>move(p)}><span><img className="navigation-art" src={`./assets/ui/navigation/${p}.png`} alt=""/></span>{label}</button>)}</nav></div>;
}

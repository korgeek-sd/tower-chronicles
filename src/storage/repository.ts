...
import type {GameState,ExpeditionLoot,Tower,MarketOrder,MarketTrade} from '../game/types';
...
function validBase(x:unknown,legacy=false,v3=false,currency:'gold'|'silver'='gold',boss=false):x is Obj {
  ...
  if(!Array.isArray(x.items)||!x.items.every(i=>obj(i)&&typeof i.id==='string'&&typeof i.kind==='string'&&(i.kind in WEAPONS||i.kind in EQUIPMENT||i.kind in PASSIVES)&&count(i.tier)&&i.tier>=1&&i.tier<=5&&[0,1,2,3].includes(i.enhancement as number)))return false;
...
export function migrateV16(value:unknown,catalog:CosmeticsCatalog=COSMETICS_CATALOG):GameState {if(!validV16(value,catalog))throw Error('v16 저장 데이터를 안전하게 이전할 수 없습니다.');const next:any=structuredClone(value);next.version=17;if(next.expedition)upgradeEvents(next.expedition);if(!validV17(next,catalog))throw Error('원정 이벤트 저장 데이터 이전 검증에 실패했습니다.');return migrateV17(next,catalog);}

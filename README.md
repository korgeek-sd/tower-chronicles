...
- 현재 production 일반 랜덤 이벤트: 전 탑 공통 6종

세계관과 시각 방향은 `WORLD-AND-ART-DIRECTION.md`를 기준으로 하며, 최신 구조 변경은 `GPT-HANDOFF-v0.1.30.md`, `GPT-HANDOFF-v0.1.31.md`, `GPT-HANDOFF-v0.1.32.md`를 함께 확인합니다.

## v0.1.32 Exploration Events

전 탑에서 재사용되는 production 탐험 이벤트 6종을 추가했습니다. 회복·보급·발견·위험·상태 이벤트가 기존 데이터 기반 이벤트 엔진을 사용하며, 선택 결과와 위험 결과 ticket은 저장 후에도 다시 굴려지지 않습니다.
...
저장 필드는 추가하지 않아 save schema는 v21을 유지합니다. 전용 이벤트 일러스트가 없는 동안 기존 중립 placeholder가 표시됩니다.
...
- 탑별 전용 production 이벤트와 전용 이벤트 일러스트
- 실제 서버 PvP
